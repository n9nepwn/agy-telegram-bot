...
async def quota_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session_info:
        ...
        parts.append(f"🤖 Modèle : `{session_info['model']}`")
        ...
        parts.append("⚪ Aucune session active\n")

    # DB stats
    if db:
        stats = await db.get_usage_stats(user_id)
        parts.append("**📊 Statistiques globales**\n")
        parts.append(f"💬 Aujourd'hui : {stats.get('messages_24h', 0)} messages")
        parts.append(f"📅 Cette semaine : {stats.get('messages_7d', 0)} messages")
        parts.append(f"📆 Ce mois : {stats.get('messages_30d', 0)} messages")
        ...
        if stats.get("total_tokens", 0) > 0:
            parts.append(f"🔢 Tokens cumulés : {stats['total_tokens']:,}")
